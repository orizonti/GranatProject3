...
void MapObject::SetMoveDirection(int Dir_X, int Dir_Y, double Lift_Step)
{
	MoveDirection.SetMoveVector(Dir_X, Dir_Y);
	MoveDirection.Step_Elevation = Lift_Step;

	ObjectImage->SetObjectDirection(MapUnitObject::DirectionTable.value(QPair<int,int>(MoveDirection.Unit_X,MoveDirection.Unit_Y)));
}
...
{
		CurrentPosition = Destination; // CHECK WHY MOVING ERROR APPEAR WITHOUT THAT STRING
	int d_x = 0;
	int d_y = 0;
	int dir_x = 0;
	int dir_y = 0;
	...
	{
		 MoveDirection.SetMoveVector(0,0); // STOP MOVING IF ROUTE POINTS EMPTY
		 return;
	}

		Destination = RoutePoints.takeFirst(); // GET INTERMEDIATE ROUTE POINT AS CURRENT DESTINATION
		...
		dir_y = d_y / std::abs(d_y);
			...
			for (auto Object : MapObjects)
				Object->SetMoveDirection(dir_x, dir_y,0);
...
void MapUnitObject::MoveUnit()
{
	...
	if (CurrentPosition == NextCell)
	{
		...
		MapObjects[0]->SetMoveDirection(MoveDirection.Unit_X,MoveDirection.Unit_Y,(CellHeightMap[2] - CellHeightMapCurrent[2]) / 40.0);
		MapObjects[1]->SetMoveDirection(MoveDirection.Unit_X,MoveDirection.Unit_Y,(CellHeightMap[3] - CellHeightMapCurrent[3]) / 40.0);
		MapObjects[2]->SetMoveDirection(MoveDirection.Unit_X,MoveDirection.Unit_Y,(CellHeightMap[0] - CellHeightMapCurrent[0]) / 40.0);
		MapObjects[3]->SetMoveDirection(MoveDirection.Unit_X,MoveDirection.Unit_Y,(CellHeightMap[1] - CellHeightMapCurrent[1]) / 40.0);
	}
